...
$total_sql = "SELECT COUNT(*) FROM orders where user_id = ?";
if ($status_filter !== 'All') {
    $total_sql .= " AND orders.status = ?";
}

$total_stmt = $conn->prepare($total_sql);
if ($status_filter !== 'All') {
    $total_stmt->bind_param("is", $user_id, $status_filter);
} else {
...
$sql = "SELECT orders.id, orders.status, orders.order_date, 
                orders.address,orders.total_price,userdata.username
            FROM orders join userdata on userdata.id=orders.user_id where orders.user_id=?";
            if ($status_filter !== 'All') {
...
<body class="bg-light">
    <div class="container-fluid">
        <div class="card shadow-lg p-4 rounded-4 border-0 bg-white">
            ...
            <div class="mb-3">
                <form method="GET" action="admin_user_orderlist.php">
                <input type="hidden" name="id" value="<?php echo $user_id;?>" />
                    ...
                    <select name="status" id="statusFilter" class="form-select w-auto d-inline">
                        ...
                        <option value="Processing" <?php if ($status_filter == 'Processing')
                            echo 'selected'; ?>>
                            Processing</option>
            ...
            <?php if ($result->num_rows > 0): ?>
                ...
                <ul class="pagination justify-content-center">
                    <?php if ($page > 1): ?>
                        <li class="page-item"><a class="page-link" href="?id=<?php echo $user_id; ?>&status=<?php echo $status_filter; ?>&page=<?php echo $page - 1; ?>">Previous</a></li>
                    <?php endif; ?>

                    <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                        <li class="page-item <?php echo $i == $page ? 'active' : ''; ?>">
                            <a class="page-link" href="?id=<?php echo $user_id; ?>&status=<?php echo $status_filter; ?>&page=<?php echo $i; ?>"><?php echo $i; ?></a>
                        </li>
                    <?php endfor; ?>

                    <?php if ($page < $total_pages): ?>
                        <li class="page-item"><a class="page-link" href="?id=<?php echo $user_id; ?>&status=<?php echo $status_filter; ?>&page=<?php echo $page + 1; ?>">Next</a></li>
                    <?php endif; ?>
                </ul>
            </nav>
            <?php else: ?>
                <h4 class="text-center text-danger">no item in orders </h4>
            <?php endif; ?>
        </div>
    </div>
</body>
